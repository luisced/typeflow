...
function baseSampleWords(count: number): string[] {
  const out: string[] = [];
  while (out.length < count) {
...
export function sampleWords(count: number, flags?: ContentFlags | null): string[] {
  return applyContentFlags(baseSampleWords(count), flags);
}
...
export function initialSampleWords(
  ...
  return applyContentFlags(initialWords(count), flags);
}
...
export function practiceSampleWords(
  ...
  const pool = WORDS.filter((w) => w.length > 0);
